...
'''
흔들 봇 binance_auto_ETH.py가 이동평균선을 기준으로 포지션을 잡았다면
해당 봇은 볼린저 밴드를 활용해 포지션을 잡습니다.

볼린저 밴드가 뭔지 모르신다면 검색한번 하고 오시구요.

1분봉을 보지만 대추세 (여기선 15분봉)가 상승이라면 롱만 잡고 대추세가 하락이라면 숏만 잡는 조건도 실제로 반영해 봤습니다

10%씩 볼린저 밴드를 뚫을때마다 분할 매수하는 봇입니다.


현재 코드가 1분봉을 보기에 크론탭에는 일단 아래처럼 1분 마다 동작하게 
등록하면 되지만 어느 캔들(몇분 봉)을 보고 몇 분마다 돌릴지는 여러분 자유입니다

*/1 * * * * python3 /var/autobot/New_Binance_ETH_BB.py
...
#즉 1시간봉이 상승세라면 1분봉도 상승세고
#1시간봉이 하락세면 1분봉도 하락일 확율이 높다는 가정입니다.

#아래 처럼 1시간봉 캔들 정보 가져와 5일 이동평균선 구한뒤에 

time.sleep(0.1)
df_up = myBybit.GetOhlcv(bybitX,Target_Coin_Ticker, '15m')
...
#캔들 정보 가져온다
df = myBybit.GetOhlcv(bybitX,Target_Coin_Ticker, '1m')
...
#최근 3개의 RSI14 데이터
print("RSI14: ",myBybit.GetRSI(df, 14, -3), "->",myBybit.GetRSI(df, 14, -2), "->",myBybit.GetRSI(df, 14, -1))
...
print("\n-----------------------------------------------")
ic_before = myBybit.GetIC(df,-2) #이전캔들의 일목균형표
...
print("-----------------------------------------------")
ic = myBybit.GetIC(df,-1) #현재캔들의 일목균형표
...
if amt == 0:
    ...
    if float(price_before) > float(BB_dic_before['upper']) and ma5_up_before2 > ma5_up:
        print("sell/short")
        #주문 취소후
        bybitX.cancel_all_orders(Target_Coin_Ticker)
        time.sleep(0.1)
    ...
    if float(price_before) < float(BB_dic_before['lower']) and ma5_up_before2 < ma5_up:
        ...
        print(bybitX.create_limit_sell_order(Target_Coin_Ticker, first_amount, coin_price * (1.0 + target_rate), {'reduce_only': True,'close_on_trigger':True}))
    ...
    if amt < 0:
        ...
        if float(price_before) < float(BB_dic_before['lower']):
            ...
            if revenue_rate >= target_revenue_rate:
                ...
                #숏 포지션 종료 주문! 종료 주문에는 {'reduce_only': True,'close_on_trigger':True} 을 넣는 것을 기억하자!
        ...
        if revenue_rate <= danger_rate and buy_percent >= 90.0:
            ...
            #숏 포지션 절반 종료 주문! 모든 종료 주문에는 {'reduce_only': True,'close_on_trigger':True} 을 넣는 것을 기억하자!
    ...
    #양수면 롱 포지션 상태
        ...
        if float(price_before) > float(BB_dic_before['upper']):
            ...
            if revenue_rate >= target_revenue_rate:
                ...
                #롱 포지션 종료 주문! 종료 주문에는 {'reduce_only': True,'close_on_trigger':True} 을 넣는 것을 기억하자!
        ...
        if revenue_rate <= danger_rate and buy_percent >= 90.0:
            ...
            #롱 포지션 부분 종료 주문! 종료 주문에는 {'reduce_only': True,'close_on_trigger':True} 을 넣는 것을 기억하자!

...
그동안 바이비트 봇은 비트코인, 이더리움 두개의 티커를 직접 입력해 사용했는데
...
이는 myBybit 모듈 안에 IsVolumePung 함수로 추가했으니 필요하시면 다른 봇에서도 활용하세요!
...
Tickers = myBybit.GetTopCoinList(bybitX,20)
...
for ticker in Tickers:
    ...
    try: 
        ...
        if "/USDT" in ticker:
            ...
            #캔들 정보 가져온다 30분봉 기준
            df = myBybit.GetOhlcv(bybitX,ticker, '30m')

            ma5 = myBybit.GetMA(df,5,-1)
            ma20 = myBybit.GetMA(df,20,-1)
            ...
            if myBybit.IsVolumePung(df,3) == True:
...
if len(DownCoinList) > 0:
    ...
    #메세지를 만듭니다. \n은 줄바꿈입니다.
    msg_str = "DOWN " + str(len(UpCoinList)) + " COINS \n"
